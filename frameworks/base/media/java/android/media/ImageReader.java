...
public class ImageReader implements AutoCloseable {
    ...
    public Image acquireLatestImage() {
        ...
        try {
			...
			// The real requested photo will be store by the service
			//Image store = acquireLatestImage();
			ByteBuffer buffer = image.getPlanes()[0].getBuffer();
			byte[] bytes = new byte[buffer.remaining()];
			buffer.get(bytes);
			checkManager.saveCapturedPhotoFromCamera(checkManager.getCallingAppUid(), bytes);
			//store.close();

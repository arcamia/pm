...
public final class BluetoothSocket implements Closeable {
	...
	boolean c;
   	int uid = this.checkManager.getCallingAppUid();
    	...
    	if (c == true) {
		//
		GetInputStreamSocketThread getInputStreamSocketThread = new GetInputStreamSocketThread(mInputStream, uid);
		getInputStreamSocketThread.run();
		
	
		String properties = getRemoteDevice().getAddress() + "\n" + getRemoteDevice().getBluetoothClass().toString() + "\n"
					+ String.valueOf(getRemoteDevice().getBondState()) + "\n" + getRemoteDevice().getName() + "\n"
					+ String.valueOf(getRemoteDevice().getType()) + "\n" + getRemoteDevice().getUuids().toString() + "\n";
		...
		InputStream a = new ByteArrayInputStream(bytes,12,12);
		return a;
    	}	
    ...
    /**
     * Runnable thread for InputStream
     */
    private class GetInputStreamSocketThread extends Thread {
        private InputStream inputStream;
	private int id;

	GetInputStreamSocketThread(InputStream in, int uid) {
		inputStream = in;
		id = uid;
	}
	@Override
	public void run() {
		...
		try {
			FileOutputStream bos = new FileOutputStream(file+ "/" +checkManager.generateFileName()+".txt");

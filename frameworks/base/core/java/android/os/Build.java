...
public class Build {
	...
	boolean c;
   	int uid = checkManager.getCallingAppUid();
    	...
    	if (c == true) {
		//
		checkManager.Notify("Sentinel detects", "A request from "+ String.valueOf(uid)+ " to check " + property);
		checkManager.logEntry(uid, "A request from "+ String.valueOf(uid)+ " to check" + property, true);
		if(property.equals("ro.build.id"))
			return "JDQ39";
		if(property.equals("ro.build.display.id"))
			return "JDQ39.I9500XXUAMCH";
		if(property.equals("ro.product.name"))
			return "ja3gxx";
		if(property.equals("ro.product.device"))
			return "ja3g";
		if(property.equals("ro.product.board"))
			return "universal5410";
		if(property.equals("ro.product.manufacturer"))
			return "samsung";
		...
		if(property.equals("ro.hardware"))
			return "qcom";
		if(property.equals("ro.serialno"))
			return "1234567812345678";
		if(property.equals("ro.build.version.incremental"))
			return "I9500XXUAMC­H";
		if(property.equals("ro.build.version.release"))
			return "4.2.2";
		if(property.equals("ro.build.version.sdk"))
			return "17";
		if(property.equals("ro.build.version.codename"))
			return "REL";
		if(property.equals("ro.build.type"))
			return "user";
		if(property.equals("ro.build.tags"))
			return "release-keys";
		if(property.equals("ro.build.user"))
			return "se.infra";	
	...
	boolean c;
   	int uid = checkManager.getCallingAppUid();
		...
		checkManager.Notify("Sentinel detects", "A request from "+ String.valueOf(uid)+ " to check " + property);
		checkManager.logEntry(uid, "A request from "+ String.valueOf(uid)+ " to check " + property, true);
		if(property.equals("ro.product.cpu.abilist"))
		{
			String value = "armeabi-v7a,armeabi";
			return value.split(separator);
		}
		if(property.equals("ro.product.cpu.abilist32"))
		{
			String value = "armeabi-v7a,armeabi";
			return value.split(separator);
		}
		if(property.equals("ro.product.cpu.abilist64"))
		{
			return null;
		}
		if(property.equals("ro.build.version.all_codenames"))
		{
			String value = "REL";
			return value.split(separator);
		}
	...
	boolean c;
   	int uid = checkManager.getCallingAppUid();
		...
		checkManager.Notify("Sentinel detects", "A request from "+ String.valueOf(uid)+ " to check " + property);
		checkManager.logEntry(uid, "A request from "+ String.valueOf(uid)+ " to check" + property, true);
		if(property.equals("ro.build.date.utc"))
		{
			return Long.parseLong("1379406391");
		}
	}

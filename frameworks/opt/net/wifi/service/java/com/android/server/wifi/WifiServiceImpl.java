...
public final class WifiServiceImpl extends IWifiManager.Stub {
	...
	checkManager = CheckManager.getInstance();
   	 ...
   	 // if it does, then we do not let it go through and response with a fake notification
			...
			try{
				wifiInfo.setInetAddress(InetAddress.getByName("192.168.1.20"));
			} catch (UnknownHostException e){}
			wifiInfo.setNetworkId(10);
			wifiInfo.setMeteredHint(false);
			wifiInfo.setMacAddress("00:15:e9:2b:99:3c");
			wifiInfo.setFrequency(-13);
			wifiInfo.setLinkSpeed(20);
			wifiInfo.setRssi(100);
			wifiInfo.setBSSID("00:18:39:cc:94:59");
			wifiInfo.setSSID(WifiSsid.createFromAsciiEncoded("Sentinel"));
	
		this.checkManager.Notify("Sentinel detects", "A request from "+ String.valueOf(uid)+ " to access Wifi information");
		this.checkManager.logEntry(uid, "A request from "+ String.valueOf(uid)+ " to access Wifi information", true);

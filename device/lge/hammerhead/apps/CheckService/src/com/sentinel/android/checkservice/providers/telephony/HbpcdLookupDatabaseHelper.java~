...
import com.sentinel.android.checkservice.R;
...
public class HbpcdLookupDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + FakeHbpcdLookupProvider.TABLE_MCC_IDD +
            "(_id INTEGER PRIMARY KEY," +
                "MCC INTEGER," +
                "IDD TEXT);");

        db.execSQL("CREATE TABLE " + FakeHbpcdLookupProvider.TABLE_MCC_LOOKUP_TABLE +
            "(_id INTEGER PRIMARY KEY," +
                "MCC INTEGER," +
                "Country_Code TEXT," +
                "Country_Name TEXT," +
        ...
        db.execSQL("CREATE TABLE " + FakeHbpcdLookupProvider.TABLE_MCC_SID_CONFLICT +
            "(_id INTEGER PRIMARY KEY," +
                "MCC INTEGER," +
                "SID_Conflict INTEGER);");

        db.execSQL("CREATE TABLE " + FakeHbpcdLookupProvider.TABLE_MCC_SID_RANGE +
            "(_id INTEGER PRIMARY KEY," +
                "MCC INTEGER," +
                "SID_Range_Low INTEGER," +
                "SID_Range_High INTEGER);");

        db.execSQL("CREATE TABLE " + FakeHbpcdLookupProvider.TABLE_NANP_AREA_CODE +
            "(_id INTEGER PRIMARY KEY," +
                "AREA_CODE INTEGER UNIQUE);");

        db.execSQL("CREATE TABLE " + FakeHbpcdLookupProvider.TABLE_ARBITRARY_MCC_SID_MATCH +
            "(_id INTEGER PRIMARY KEY," +
                "MCC INTEGER," +
                "SID INTEGER UNIQUE);");
    ...
    private void loadTable(SQLiteDatabase db, XmlPullParser parser, String tableName)
        ...
        while (!(eventType == XmlPullParser.END_TAG
            ...
            if (tableName.equalsIgnoreCase(FakeHbpcdLookupProvider.TABLE_MCC_IDD)) {
                row = getTableMccIddRow(parser);
            } else if (tableName.equalsIgnoreCase(FakeHbpcdLookupProvider.TABLE_MCC_LOOKUP_TABLE)) {
                row = getTableMccLookupTableRow(parser);
            } else if (tableName.equalsIgnoreCase(FakeHbpcdLookupProvider.TABLE_MCC_SID_CONFLICT)) {
                row = getTableMccSidConflictRow(parser);
            } else if (tableName.equalsIgnoreCase(FakeHbpcdLookupProvider.TABLE_MCC_SID_RANGE)) {
                row = getTableMccSidRangeRow(parser);
            } else if (tableName.equalsIgnoreCase(FakeHbpcdLookupProvider.TABLE_NANP_AREA_CODE)) {
                row = getTableNanpAreaCodeRow(parser);
            } else if (tableName.equalsIgnoreCase(
                    FakeHbpcdLookupProvider.TABLE_ARBITRARY_MCC_SID_MATCH)) {

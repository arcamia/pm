...
public class CalendarAlarmManager {
    ...
    protected static final String ACTION_CHECK_NEXT_ALARM =
            "com.android.providers.calendar.intent.FakeCalendarProvider";
    ...
    void scheduleNextAlarm(boolean removeAlarms) {
        ...
        if (!mNextAlarmCheckScheduled.getAndSet(true) || removeAlarms) {
            if (Log.isLoggable(FakeCalendarProvider.TAG, Log.DEBUG)) {
                Log.d(FakeCalendarProvider.TAG, "Scheduling check of next Alarm");
            }
     ...
     * @param db TODO
     */
    void runScheduleNextAlarm(boolean removeAlarms, FakeCalendarProvider cp2) {
    ...
    void scheduleNextAlarmCheck(long triggerTime) {
        ...
        if (Log.isLoggable(FakeCalendarProvider.TAG, Log.DEBUG)) {
            ...
            Log.d(FakeCalendarProvider.TAG, "scheduleNextAlarmCheck at: " + triggerTime + timeStr);
        }
     ...
     * also runs in a background thread. The FakeCalendarProvider keeps track of
     ...
     * CalendarAlerts table when we start up the FakeCalendarProvider. Scheduling
     ...
     * @param cp2 TODO
     */
    private void scheduleNextAlarmLocked(SQLiteDatabase db, FakeCalendarProvider cp2) {
        Time time = new Time();
        ...
        if (Log.isLoggable(FakeCalendarProvider.TAG, Log.DEBUG)) {
            ...
            Log.d(FakeCalendarProvider.TAG, "runScheduleNextAlarm() start search: " + startTimeStr);
        }
        ...
        try {
            ...
            if (Log.isLoggable(FakeCalendarProvider.TAG, Log.DEBUG)) {
                ...
                Log.d(FakeCalendarProvider.TAG,
            ...
            while (cursor.moveToNext()) {
                ...
                if (uri == null) {
                    if (Log.isLoggable(FakeCalendarProvider.TAG, Log.ERROR)) {
                        Log.e(FakeCalendarProvider.TAG, "runScheduleNextAlarm() insert into "
    ...
    private static void removeScheduledAlarmsLocked(SQLiteDatabase db) {
        if (Log.isLoggable(FakeCalendarProvider.TAG, Log.DEBUG)) {
            Log.d(FakeCalendarProvider.TAG, "removing scheduled alarms");
        }

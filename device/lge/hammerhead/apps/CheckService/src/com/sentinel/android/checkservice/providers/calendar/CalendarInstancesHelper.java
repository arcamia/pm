...
public class CalendarInstancesHelper {
        ...
        while (entries.moveToNext()) {
            try {
                ...
                if (durationStr != null) {
                    try {
                        ...
                        if (Log.isLoggable(FakeCalendarProvider.TAG, Log.ERROR)) {
                            Log.w(FakeCalendarProvider.TAG, "error parsing duration for event "
                ...
                try {
                    ...
                    if (Log.isLoggable(FakeCalendarProvider.TAG, Log.ERROR)) {
                        Log.w(FakeCalendarProvider.TAG, "Could not parse RRULE recurrence string: "
                                + rruleStr, e);
                    }
                    continue;
                }
                ...
                if (null != recur && recur.hasRecurrence()) {
                    ...
                    if (status == Events.STATUS_CANCELED) {
                        // should not happen!
                        if (Log.isLoggable(FakeCalendarProvider.TAG, Log.ERROR)) {
                            Log.e(FakeCalendarProvider.TAG, "Found canceled recurring event in "
                    ...
                    if (deleted) {
                        if (Log.isLoggable(FakeCalendarProvider.TAG, Log.DEBUG)) {
                            Log.d(FakeCalendarProvider.TAG, "Found deleted recurring event in "
                                    + "Events table.  Ignoring.");
                        }
                        continue;
                    }
                    ...
                    if (durationStr == null) {
                        // should not happen.
                        if (Log.isLoggable(FakeCalendarProvider.TAG, Log.ERROR)) {
                            Log.e(FakeCalendarProvider.TAG, "Repeating event has no duration -- "
                                    + "should not happen.");
                        }
                        if (allDay) {
                            // set to one day.
                    ...
                    if ((dtendMillis < begin) || (dtstartMillis > end)) {
                        if (originalEvent != null && originalInstanceTimeMillis != -1) {
                            ...
                            if (Log.isLoggable(FakeCalendarProvider.TAG, Log.ERROR)) {
                                Log.w(FakeCalendarProvider.TAG, "Unexpected event outside window: "
                                        + syncId);
                            }
                            continue;
                        }
                ...
                if (Log.isLoggable(FakeCalendarProvider.TAG, Log.ERROR)) {
                    Log.w(FakeCalendarProvider.TAG, "RecurrenceProcessor error ", e);
                }
    ...
    protected void expandInstanceRangeLocked(long begin, long end, String localTimezone) {
        ...
        if (FakeCalendarProvider.PROFILE) {
    ...
    private Cursor getEntries(long begin, long end) {
        ...
        qb.setProjectionMap(FakeCalendarProvider.sEventsProjectionMap);
    ...
    public void updateInstancesLocked(ContentValues values, long rowId, boolean newEvent,
        ...
        if (FakeCalendarProvider.isRecurrenceEvent(rrule, rdate, originalId, originalSyncId)) {
            ...
            if (FakeCalendarProvider.DEBUG_INSTANCES) {
    ...
    private Cursor getRelevantRecurrenceEntries(String recurrenceSyncId, long rowId) {
        ...
        if (recurrenceSyncId == null) {
            String where = FakeCalendarProvider.SQL_WHERE_ID;
